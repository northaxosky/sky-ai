...
def test_mlp_shape_preserving():
    """MLP should preserve (B, T, n_embd) shape"""
    mlp = MLP(n_embd=64)
...
def test_mlp_hidden_dim_8_3_with_alignment():
    ...
    mlp = MLP(n_embd=768, hidden_multiple=4, align=256)
    assert mlp.gate_proj.out_features == 2048
    assert mlp.up_proj.out_features == 2048

    mlp_xl = MLP(n_embd=1600, hidden_multiple=4, align=256)
...
def test_mlp_param_count_matched_to_gelu():
    ...
    n = 768
    mlp = MLP(n_embd=n, hidden_multiple=4, align=256)
