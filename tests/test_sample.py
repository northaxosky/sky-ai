...
def tiny_model() -> GPT:
    return GPT(GPTConfig(
        n_layer=1, n_head=2, n_embed=32,
        vocab_size=50257, block_size=16,
    ))


class TestSample:
    def test_returns_n_samples_strings(self, tiny_model: GPT,
                                        encoder: tiktoken.Encoding) -> None:
        out = sample(tiny_model, encoder, "Hello",
                    n_samples=3, max_length=8, device="cpu")
        assert len(out) == 3
        assert all(isinstance(s, str) for s in out)

    def test_each_completion_starts_with_prompt(self, tiny_model: GPT,
                                                encoder: tiktoken.Encoding) -> None:
        out = sample(tiny_model, encoder, "Hello",
                    n_samples=2, max_length=8, device="cpu")
    ...
    def test_max_length_translates_to_max_new_tokens(
        ...
        sample(tiny_model, encoder, "Hello",
            n_samples=1, max_length=6, device="cpu")
        assert captured["max_new_tokens"] == 6 - captured["prompt_len"]

    def test_top_k_none_does_not_crash(self, tiny_model: GPT,
                                        encoder: tiktoken.Encoding) -> None:
        out = sample(tiny_model, encoder, "Hi",
                    n_samples=1, max_length=4, device="cpu", top_k=None)
        assert len(out) == 1
    ...
    def test_prompt_longer_than_max_length_still_generates_one_token(
        ...
        out = sample(tiny_model, encoder, "Hello, I'm a language model,",
                    n_samples=1, max_length=4, device="cpu")
        assert len(out) == 1
        assert out[0].startswith("Hello, I'm a language model,")

    def test_raises_on_invalid_n_samples(self, tiny_model: GPT,
                                        encoder: tiktoken.Encoding) -> None:
        with pytest.raises(ValueError, match="n_samples"):
            sample(tiny_model, encoder, "Hi",
                n_samples=0, max_length=4, device="cpu")

    def test_seeded_generator_makes_output_deterministic(
        self, tiny_model: GPT, encoder: tiktoken.Encoding,
    ) -> None:
        rng1 = torch.Generator(device="cpu").manual_seed(123)
        out1 = sample(tiny_model, encoder, "Hi",
                    n_samples=2, max_length=8, device="cpu", generator=rng1)
        rng2 = torch.Generator(device="cpu").manual_seed(123)
        out2 = sample(tiny_model, encoder, "Hi",
                    n_samples=2, max_length=8, device="cpu", generator=rng2)
        assert out1 == out2
